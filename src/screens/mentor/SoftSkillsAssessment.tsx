...
import { getCategoriesSoftSkillsApi, submitSoftSkillsAssessmentApi } from "../../services/mentorApi";
...
const SoftSkillsAssessment = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    mentor_id: Yup.string().required("Mentor is required"),
  ...
  const handleSubmit = async (values: any, { setSubmitting, resetForm }: any) => {
    ...
    try {
      ...
      await submitSoftSkillsAssessmentApi(payload);
          ...
          return (
            <Form>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                <div>
                  <h1 className="text-xl font-extrabold text-navy">Soft Skills Assessment</h1>
                  <p className="text-xs text-mist mt-0.5">
                    Evaluate intern soft skills across {categories.length} categories · Total {maxTotal} points
                  </p>
                </div>
                      ...
                      <div key={cat.categoryId} className="bg-white border border-line rounded-2xl overflow-hidden transition-shadow hover:shadow-sm">
                        <div className="flex items-center justify-between px-5 py-4">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-8 h-8 rounded-lg bg-emerald-50 flex items-center justify-center text-emerald-600 text-sm font-extrabold flex-shrink-0">{catIdx + 1}</div>
                        ...
                        <div className="px-5 pb-5 border-t border-line pt-4">
                          <Field
                            ...
                            style={{ background: `linear-gradient(to right, #059669 ${(values.category_marks[catIdx]?.marks / cat.totalMarks) * 100}%, #f0f2f5 ${(values.category_marks[catIdx]?.marks / cat.totalMarks) * 100}%)` }}
                          />
                        </div>
                      </div>
                    ))
