...
import { getCategoriesSoftSkillsApi } from "../../services/mentorApi";
...
const SoftSkillsAssessment = () => {
    ...
    useEffect(() => {
      const fetchBatches = async () => {
        ...
        try {
          ...
          // Try to map to {id, name} structure
  ...
  const getCategories = async () => {
    ...
    try {
      const res = await getCategoriesSoftSkillsApi(id || 1); // fallback to 1 for soft skills
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-xs text-mist mt-0.5">
            Evaluate intern soft skills across 6 categories · Total {maxTotal}{" "}
            points
          </p>
        </div>
      ...
      <div className="bg-white border border-line rounded-2xl p-5 mb-5 flex flex-col sm:flex-row items-center gap-5">
        ...
        <div className="flex-1 w-full sm:w-auto space-y-1.5 sm:pl-6 sm:border-l sm:border-line">
          {categories.map((cat, i) => {
            const pct =
              cat.totalMarks > 0
                ? Math.round((scores[i] / cat.totalMarks) * 100)
                : 0;
            return (
          ...
          >
            ...
            <div className="px-5 pb-5 border-t border-line pt-4">
              <input
                ...
                onChange={(e) =>
                  handleScoreChange(catIdx, Number(e.target.value))
                }
                ...
                style={{
                  background: `linear-gradient(to right, #059669 ${(scores[catIdx] / cat.totalMarks) * 100}%, #f0f2f5 ${(scores[catIdx] / cat.totalMarks) * 100}%)`,
                }}
              />
            </div>
          </div>
