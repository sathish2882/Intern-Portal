...
// API for submitting presentation assessment
const submitPresentationAssessmentApi = (data: any) => {
  return API.post("/mentor_assessment/presentation", data);
};
...
const PresentationAssessment = () => {
  ...
  useEffect(() => {
    const getCategories = async () => {
      ...
      try {
        const res = await getCategoriesPresentationApi(id || 0);
  ...
  const validationSchema = Yup.object().shape({
    ...
    remarks: Yup.string(),
    presentation_topic: Yup.string().required("Presentation topic required"),
  ...
  const handleSubmit = async (values: any, { setSubmitting, resetForm }: any) => {
    ...
    try {
      ...
      await submitPresentationAssessmentApi(payload);
          ...
          return (
            <Form>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                <div>
                  <h1 className="text-xl font-extrabold text-navy">Presentation Assessment</h1>
              ...
              <div className="bg-white border border-line rounded-2xl p-5 mb-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate mb-1">Presentation Topic *</label>
                    <Field
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate mb-1">Specialization</label>
                    <Field
                      type="text"
                      name="specialization"
                      ...
                      <div key={cat.categoryId} className="bg-white border border-line rounded-2xl overflow-hidden transition-shadow hover:shadow-sm">
                        <div className="flex items-center justify-between px-5 py-4">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center text-amber-600 text-sm font-extrabold flex-shrink-0">{catIdx + 1}</div>
                        ...
                        <div className="px-5 pb-5 border-t border-line pt-4">
                          <Field
                            ...
                            style={{ background: `linear-gradient(to right, #d97706 ${(values.category_marks[catIdx]?.marks / cat.totalMarks) * 100}%, #f0f2f5 ${(values.category_marks[catIdx]?.marks / cat.totalMarks) * 100}%)` }}
                          />
                        </div>
                      </div>
                    ))
