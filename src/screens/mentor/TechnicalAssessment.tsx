...
const TechnicalAssessment = () => {
  ...
  useEffect(() => {
    const getCategories = async () => {
      ...
      try {
        const res = await getCategoriesTechnicalApi(id || 2); // fallback to 2 for demo
        setCategories(res.data);
        setScores(res.data.map(() => 0));
      } catch (err) {
  ...
  const getCategories = async () => {
    setLoadingCategories(true);
    try {
      const res = await getCategoriesTechnicalApi(id || "");
      const formatCategories = res.data.map((cat: any) => ({
            ...
            >
              ...
              <div className="px-5 pb-5 border-t border-line pt-4">
                <input
                  ...
                  style={{ background: `linear-gradient(to right, #1d6ede ${(scores[catIdx] / cat.totalMarks) * 100}%, #f0f2f5 ${(scores[catIdx] / cat.totalMarks) * 100}%)` }}
                />
              </div>
            </div>
          );
