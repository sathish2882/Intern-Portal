...
const normalizeUsers = (payload: unknown): BatchUser[] => {
  return toArray(payload)
    .map((item) => {
      ...
      const name =
        user.name ?? user.username ?? user.full_name ?? user.fullName;
    ...
    <Formik<SendEmailFormValues>
      ...
      onSubmit={async (values) => {
        try {
          ...
          console.error(error);
          toast.error(
            error?.response?.data?.detail || "Failed to send payment email",
          );
        } finally {
          setSending(false);
        }
      ...
      {({ values, errors, touched, setFieldValue, setFieldTouched }) => {
        const handleBatchChange = async (batchId: string) => {
          ...
          try {
            ...
            console.error(error);
            toast.error(
              error?.response?.data?.detail || "Failed to load batch users",
            );
          } finally {
        ...
        return (
          <Form className="text-adark send-email-form">
            <div className="flex flex-col gap-4 mb-6 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <h1 className="text-2xl font-extrabold text-adark mb-1">
                  Send Email
                </h1>
                <p className="text-sm text-amuted">
                  Compose and send payment emails to your user.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
              <div className="bg-abg2 border border-white/[0.07] rounded-2xl p-5">
                <h2 className="text-lg font-bold text-adark mb-4">
                  Email Details
                </h2>
                ...
                <div className="bg-abg3 border border-white/[0.07] rounded-xl p-4">
                  <div className="flex items-center gap-3 pb-3 mb-3 border-b border-white/[0.07]">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-amuted font-mono">
                        payments@mguru.com
                      </p>
                    </div>
                  </div>
                  ...
                  <p className="text-sm font-bold text-adark mb-3">
                    {titles[emailType]}
                  </p>
                  ...
                  {(values.accountName ||
                    ...
                    <div className="text-xs text-amuted bg-white/[0.03] rounded-lg p-2.5 mb-3 space-y-1">
                      <p className="text-[11px] font-bold text-adark uppercase tracking-wide mb-1">
                        Bank Details
                      </p>
                      {values.bankName && (
                        <p>
                          Bank:{" "}
                          <span className="text-adark">{values.bankName}</span>
                        </p>
                      )}
                      {values.accountName && (
                        <p>
                      ...
                      {values.accountNo && (
                        <p>
                          A/C No:{" "}
                          <span className="text-adark">{values.accountNo}</span>
                        </p>
                      )}
                      {values.ifsc && (
                        <p>
                          IFSC:{" "}
                          <span className="text-adark">{values.ifsc}</span>
                        </p>
                      )}
                    </div>
                  )}
                  {values.note && (
                    <p className="text-sm text-amuted bg-white/[0.03] rounded-lg p-2.5">
                      {values.note}
                    </p>
                  )}
                </div>
