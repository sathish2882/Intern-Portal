...
const normalizeDashboard = (payload: unknown): DashboardResponse => {
  ...
  return {
    ...
    active_customers: Number(
      data.active_customers ?? data.total_customers ?? 0,
    ),
    overdue: Number(data.overdue ?? data.pending_overdue ?? 0),
    recent_mail: normalizeRecentMail(
      data.recent_mail ?? data.recent_emails ?? [],
    ),
    email_type_count: normalizeEmailTypeCount(
      data.email_type_count ?? data.email_counts ?? [],
    ),
  };
};
...
const AdminDashboard = () => {
  ...
  const statCards = [
    {
      label: "Email Sent",
      value: dashboard?.email_sent ?? 0,
      valueClass: "text-gold",
    },
    {
      label: "Total Invoice",
      value: formatCurrency(dashboard?.total_invoice ?? 0),
      valueClass: "text-asuccess",
    },
    {
      label: "Active Customers",
      value: dashboard?.active_customers ?? 0,
      valueClass: "text-ainfo",
    },
    {
      label: "Overdue",
      value: dashboard?.overdue ?? 0,
      valueClass: "text-adanger",
    },
  ];
    ...
    <div className="text-adark">
      <div className="flex flex-col gap-4 mb-7 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="text-sm text-amuted">Payment email overview.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-6">
        {statCards.map((card) => (
          <div
            key={card.label}
            className="bg-abg2 border border-white/[0.07] rounded-2xl p-4 lg:p-5"
          >
            <p className="text-[11px] text-amuted uppercase tracking-[0.07em] mb-2">
              {card.label}
            </p>
      ...
      <div className="flex flex-col gap-7">
        <div className="xl:col-span-2 bg-abg2 border border-white/[0.07] rounded-2xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[13.5px]">
              <thead>
                <tr className="border-b border-white/[0.07]">
                  {[
                    ...
                    <th
                      key={heading}
                      className="text-left text-xs px-4 py-3 text-[11px] font-mono uppercase tracking-[0.06em] text-amuted font-medium"
                    >
                      {heading}
                    </th>
                  ))}
              ...
              <tbody>
                {!loadingDashboard &&
                  (dashboard?.recent_mail.length ?? 0) === 0 && (
                    <tr>
                      <td
                        colSpan={6}
                        className="px-4 py-6 text-sm text-amuted text-center"
                      >
                        No emails found.
                      </td>
                    </tr>
                ...
                {dashboard?.recent_mail.map((mail) => (
                  <tr
                    key={mail.invoice_no}
                    className="border-b border-white/[0.04] hover:bg-abg3 transition-colors"
                  >
                    <td className="px-4 py-3 align-middle">
                      <p className="font-semibold text-sm text-adark leading-none">
                        {mail.invoice_no}
                      </p>
                      <p className="text-xs text-amuted mt-1">
                    ...
                    <td className="px-4 py-3 align-middle">
                      <span
                        className={`text-sm font-bold px-2.5 py-0.5 rounded-full ${mail.is_complete ? "bg-asuccess/10 text-asuccess border border-asuccess/25" : "bg-gold/10 text-gold border border-gold/25"}`}
                      >
        ...
        <div className="bg-abg2 border border-white/[0.07] rounded-2xl p-5">
          ...
          <div className="space-y-5">
            {!loadingDashboard &&
              (dashboard?.email_type_count.length ?? 0) === 0 && (
            ...
            {dashboard?.email_type_count.map((item) => {
              const pct =
                totalTypeCount > 0
                  ...
                  : 0;
              const colorClass =
                EMAIL_TYPE_COLOR[getEmailTypeLabel(item.email_type)] ??
                "bg-white/30";

              return (
                <div key={`${item.email_type}-${item.count}`}>
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-sm font-semibold text-adark">
                      {formatLabel(item.email_type)}
                    </span>
                    <span className="text-[11px] text-amuted">
                      {item.count}
                    </span>
                  </div>
